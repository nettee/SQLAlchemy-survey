...
面向外部的部分是`Session`和用户定义的对象的集合，每个用户定义的对象都是一个映射类的实例。这里我们看到，映射的对象保存了一个到`InstanceState`的引用，这个对象记录了ORM的状态，包括即将发生的属性改变和属性消除状态。`InstanceState`是在前面*Anatomy of a Mapping*章节讨论的属性instrumentation的实例级一边。
